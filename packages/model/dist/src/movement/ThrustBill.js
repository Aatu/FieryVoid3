...
import { RequiredThrust, ThrustAssignment } from "./index";
class ThrustBill {
    ship;
    movement;
    thrusters;
    ...
    constructor(ship, movement) {
        ...
        this.thrusters = ship.systems
            .getSystems()
            .filter((system) => system.callHandler(SYSTEM_HANDLERS.isThruster, null, false))
    ...
    getCurrentThrustRequired() {
        return (this.directionsRequired[0] +
    ...
    totalFuelRequirement() {
        return this.thrusters.reduce((all, thruster) => all + thruster.getFuelRequirement(), 0);
    }
    getAllUsableThrusters(direction) {
        return this.thrusters
            .filter((thruster) => {
            const capacity = thruster.getThrustCapacity();
            return (thruster.isThrustDirection(direction) &&
                capacity > 0 &&
    ...
    sortThrusters(a, b) {
        const { overheatPercentage: aHeat, coolingPercent: aCooling } = a.getOverheat();
        const { overheatPercentage: bHeat, coolingPercent: bCooling } = b.getOverheat();
        if (aHeat > bHeat) {
            return -1;
        }
        if (aHeat < bHeat) {
    ...
    buildRequiredThrust(movement) {
        movement.forEach((move) => move.setRequiredThrust(new RequiredThrust().calculate(this.ship, move)));
    }
    getMoves() {
