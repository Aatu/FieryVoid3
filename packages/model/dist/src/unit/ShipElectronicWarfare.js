...
import ShipCurrentElectronicWarfare from "./ShipCurrentElectronicWarfare";
...
class ShipElectronicWarfare {
    ...
    activatePlannedElectronicWarfare() {
        this.inEffect = new ShipCurrentElectronicWarfare(this.getDefensiveEw(), this.getCcEw(), this.getAllEntries());
    }
    ...
    getEw(type, target) {
        ...
        if (type === EW_TYPE.DEFENSIVE) {
            return this.getEwArrays().reduce((total, system) => total +
                system.callHandler(SYSTEM_HANDLERS.getTotalEwUsedByType, type, 0), 0);
        }
    ...
    getAllOew() {
        ...
        this.getEwArrays()
            .reduce((all, system) => [
            ...all,
            ...system.callHandler(SYSTEM_HANDLERS.getEwEntries, null, []),
        ], [])
    ...
    getAllEntries() {
        return this.getEwArrays().reduce((all, system) => [
            ...all,
            ...system.callHandler(SYSTEM_HANDLERS.getEwEntries, null, []),
        ], []);
    }
    getEwArrays() {
        return this.ship.systems
            .getSystems()
            .filter((system) => system.callHandler(SYSTEM_HANDLERS.isEwArray, null, false));
    }
    repeatElectonicWarfare() {
        this.assignEntries(this.getAllEntries(), true);
    }
    removeAll() {
        this.getEwArrays().forEach((system) => system.callHandler(SYSTEM_HANDLERS.resetEw, null, false));
    }
    assignEntries(entries, allowIncomplete = false) {
        ...
        entries
            ...
            .map((entry) => {
            negativeEntries
                .filter((negativeEntry) => negativeEntry.type === entry.type &&
                ...
                else if (Math.abs(negativeEntry.getAmount()) > entry.getAmount()) {
    ...
    assignPositiveEW(entry, allowIncomplete) {
        ...
        while (amount > 0) {
            ...
            availableSystems.shift()?.callHandler(SYSTEM_HANDLERS.assignEw, {
                type: entry.type,
    ...
    getAvailableSystemsForEntry(entry) {
        return this.getEwArrays()
            .filter((system) => system.callHandler(SYSTEM_HANDLERS.canUseEw, {
            type: entry.type,
            amount: 1,
        }, undefined))
            .sort((a, b) => {
            const aValidTypes = a.callHandler(SYSTEM_HANDLERS.getValidEwTypes, undefined, []);
            const bValidTypes = b.callHandler(SYSTEM_HANDLERS.getValidEwTypes, undefined, []);
