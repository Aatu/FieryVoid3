...
class SystemHeat {
    ...
    heat;
    overheat;
    heatTransferPerStructure;
    overheatLimitPerStructure;
    ...
    getHeatTransferPerStructure() {
        return (this.system.callHandler(SYSTEM_HANDLERS.getHeatTransferPerStructure, null, 0) + this.heatTransferPerStructure);
    }
    getHeat() {
        return this.heat;
    }
    ...
    getOverheatTransferRatio() {
        return (this.system.callHandler(SYSTEM_HANDLERS.getOverheatTransferRatio, null, 0) || this.overheatTransferRatio);
    }
    getTransferOverHeat() {
    ...
    shouldDisplayHeat() {
        return (this.system.callHandler(SYSTEM_HANDLERS.generatesHeat, null, false) ||
            this.isHeatStorage());
    }
    predictHeatChange() {
        ...
        const maxOverheatCooling = heatCooling < cooling ? cooling - heatCooling : 0;
        const overheatCooling = maxOverheatCooling > this.overheat * this.getOverheatTransferRatio()
            ? this.overheat * this.getOverheatTransferRatio()
            : maxOverheatCooling;
        const cumulatedOverheat = this.overheat + finalHeat;
        const actualOverheatCooling = overheatCooling <= cumulatedOverheat
            ? overheatCooling
            : cumulatedOverheat;
    ...
    getRadiateHeatCapacity() {
        return this.system.callHandler(SYSTEM_HANDLERS.getHeatRadiationCapacity, null, 0);
    }
    radiateHeat(heat) {
        return this.system.callHandler(SYSTEM_HANDLERS.radiateHeat, heat, undefined);
    }
    advanceTurn(turn) {
        this.heatTransferred = 0;
    }
