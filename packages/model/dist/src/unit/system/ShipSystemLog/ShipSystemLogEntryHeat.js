...
class ShipSystemLogEntryHeat extends ShipSystemLogEntry {
    ...
    getMessage() {
        let messages = [];
        const isRadiator = this.system.callHandler(SYSTEM_HANDLERS.isRadiator, null, false);
    ...
    getHeatStorageMessage() {
        ...
        if (this.heatGivenToRadiators || this.newHeatStored) {
            messages.push(`Added ${formatNumber(this.newHeatStored)} units of heat. Transfered ${formatNumber(this.heatGivenToRadiators)} units of heat to radiators.`);
        }
        messages.push(`Stored ${formatNumber(this.intialHeatStored + this.newHeatStored - this.heatGivenToRadiators)} units of heat.`);
        return messages;
    }
    getRegularMessage() {
        const messages = [];
        if (this.heatGenerated || this.heatCooled) {
            messages.push(`Added ${formatNumber(this.heatGenerated)} and cooled ${formatNumber(this.heatCooled)} units of heat.`);
        }
        let heatMessage = `Current system heat was ${formatNumber(this.overheat)}.`;
        messages.push();
        if (this.overheat) {
            heatMessage += ` Overheating ${Math.round(this.overheatPercentage * 100)}%.`;
        }
