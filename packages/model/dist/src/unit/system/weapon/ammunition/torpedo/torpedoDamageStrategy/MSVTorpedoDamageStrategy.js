...
export const isMSVTorpedoDamagePayload = (payload) => {
    return !!payload.torpedoFlight;
};
class MSVTorpedoDamageStrategy extends StandardDamageStrategy {
    ...
    minStrikeDistance;
    msv;
    constructor(damageFormula, armorPiercingFormula, rangePenalty, numberOfShots, strikeHitChance = 20, minStrikeDistance = 1) {
    ...
    _getDamageForWeaponHit({ torpedoFlight }) {
        ...
        return Math.ceil(this.diceRoller.roll(this.damageFormula));
    }
    ...
    getHitChance({ target, torpedoFlight, distance, }) {
        ...
        const rangeModifier = this.rangePenalty * (1 + target.movement.getEvasion() / 10) * distance;
    ...
    applyDamageFromWeaponFire(payload) {
        ...
        combatLogEntry.addNote(`MSV with ${this.numberOfShots} projectiles at distance ${distance} with hit chance of ${hitChance}% each.`);
        let hits = 0;
        while (shots--) {
            ...
            if (hit) {
                ...
                this.doDamage({ shooterPosition: attackPosition, ...payload }, result, null);
            }
        }
