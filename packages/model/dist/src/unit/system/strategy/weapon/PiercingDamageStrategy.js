...
class PiercingDamageStrategy extends StandardDamageStrategy {
    ...
    getMessages(payload, previousResponse = []) {
        ...
        previousResponse.push({
            value: "Piercing will cause damage multiple times applying new damage each time, until it runs out of armor piercing." +
    ...
    chooseHitSystem = ({ target, shooterPosition, systemsHit }) => {
    ...
    doDamage(payload, damageResult, lastSection, armorPiercing, inputDamage, shotsResolved, systemsHit = []) {
        const { target, shooterPosition, combatLogEntry } = payload;
        if (armorPiercing === 0) {
            combatLogEntry.addNote(`Pierced ${shotsResolved} times. Ran out of armor piercing.`);
            return;
        }
        ...
        let result = this.doDamageToSystem(payload, damageResult, hitSystem, armorPiercing, this.getDamageForWeaponHit(payload));
        armorPiercing = result.armorPiercing;
        if (!(hitSystem instanceof Structure)) {
            systemsHit.push(hitSystem);
        }
        return this.doDamage(payload, damageResult, null, armorPiercing, 0, shotsResolved !== undefined ? shotsResolved + 1 : 1, systemsHit);
    }
}
