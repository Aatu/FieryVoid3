...
export const storeAngleAndChange = (angle, change) => {
...
export const retrieveAngleAndChange = (data) => {
    const change = radianToDegree(((data & first16ByteMask) - 16383) / 1000);
    const angle = radianToDegree((((data & second16ByteMask) >> 15) - 16383) / 100);
    return { angle, change };
};
export const storeOpacityAndFade = (opacity, fadeInSpeed, fadeOutSpeed) => {
    ...
    return ((Math.round(fadeOutSpeed) << 20) +
        (Math.round(fadeInSpeed) << 8) +
        finalOpacity);
};
export const retrieveOpacityAndFade = (data) => {
