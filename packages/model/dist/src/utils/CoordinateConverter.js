...
import { isIOffset } from "../hexagon/Offset";
...
export class CoordinateConverter {
    ...
    fromGameToHex(gameCoordinates) {
        const q = ((1 / 3) * Math.sqrt(3) * gameCoordinates.x -
    ...
    fromHexToGame(offsetHex) {
        ...
        const x = this.hexlenght * Math.sqrt(3) * (offsetHex.q - 0.5 * (offsetHex.r & 1));
    ...
    fromViewPortToGame(pos) {
        ...
        this.raycaster.setFromCamera(new THREE.Vector2(pos.xR, pos.yR), this.getCamera().getCamera());
        const intersects = this.raycaster.intersectObjects(this.getScene().children, true);
    ...
    getEntitiesIntersected(pos) {
        const result = [];
        this.raycaster.setFromCamera(new THREE.Vector2(pos.xR, pos.yR), this.getCamera().getCamera());
        var intersects = this.raycaster.intersectObjects(this.getScene().children, true);
        intersects.forEach(function (intersected) {
            if (intersected.object.name !== "hexgrid") {
