...
const getSeededRandomGenerator = (seed) => {
    ...
    return xoshiro128ss(seedGenerator(), seedGenerator(), seedGenerator(), seedGenerator());
};
...
const getPointBetween = (start, end, percentage, noRound = false) => {
...
const getPointBetween3d = (start, end, percentage, noRound = false) => {
    var x = start.x + percentage * (end.x - start.x);
    var y = start.y + percentage * (end.y - start.y);
...
const getPointBetweenInDistance = (start, end, distance, noRound = false) => {
...
const getAngleBetween = (angle1, angle2, right = false) => {
    let difference;
    if (right) {
        if (angle1 > angle2) {
...
const getPointInDirection = (r, a, cx, cy, noRound = false) => {
    a = -a;
...
const getCompassHeadingOfPoint = (observer, target) => {
    let heading = radianToDegree(Math.atan2(target.y - observer.y, target.x - observer.x));
    if (heading > 0) {
        heading = 360 - heading;
    }
    else {
