...
class TorpedoAttackService {
    ...
    getPossibleInterceptors(ship, torpedoFlight) {
        return ship.systems
            ...
            return weapon.callHandler(SYSTEM_HANDLERS.isPositionOnArc, { targetPosition: torpedoFlight.strikePosition }, true);
        })
            .filter((weapon) => {
            const fireOrders = weapon.callHandler(SYSTEM_HANDLERS.getFireOrders, null, []);
            return fireOrders.length === 0;
        });
    }
    getTorpedoLaunchOptions(ship, target) {
    ...
    getPossibleTorpedos(currentUser, target) {
        ...
        this.getGameData()
            ...
            ship.systems.getSystems().forEach((system) => {
                const launchers = system.callHandler(SYSTEM_HANDLERS.getLoadedLaunchers, null, []);
