import MovementOrder from "../movement/MovementOrder";
...
class ShipMovement {
    ...
    removeMovementExceptEnd(turn) {
        this.moves = this.moves.filter((move) => move.turn < turn || move.isEnd() || move.isDeploy() || move.isStart());
        this.buildIndex();
    }
    ...
    buildIndex() {
        ...
        this.ship.systems.callAllSystemHandlers(SYSTEM_HANDLERS.resetChanneledThrust, undefined);
        this.moves.forEach((move) => {
    ...
    getThrustOutput() {
        return this.ship.systems
            .getSystems()
            .reduce((acc, system) => system.callHandler(SYSTEM_HANDLERS.getThrustOutput, null, acc), 0);
    }
    getThrustRequired() {
        return this.moves
            .filter((move) => move.requiredThrust)
            .reduce((total, move) => total + move.requiredThrust.getTotalAmountRequired(), 0);
    }
    ...
    getThrusters() {
        return this.ship.systems
            .getSystems()
            .filter((system) => system.callHandler(SYSTEM_HANDLERS.isThruster, undefined, false))
            .filter((system) => !system.isDisabled());
    }
    getMaxEvasion() {
        return this.ship.systems
            .getSystems()
            .reduce((acc, system) => system.callHandler(SYSTEM_HANDLERS.getMaxEvasion, null, acc), 0);
    }
    /*
    ...
    revertMovementsUntilValidMovement() {
        const canRevert = () => this.getMovement().some((move) => move.isPlayerAdded());
        while (true) {
            ...
            if (!canRevert()) {
                throw new Error("Ship does not have valid movement, but can not revert either");
            }
    ...
    getFuel() {
        return this.ship.systems
            .getSystems()
            .reduce((all, system) => all + system.callHandler(SYSTEM_HANDLERS.getFuel, null, 0), 0);
    }
    getFuelSpace() {
        return this.ship.systems
            .getSystems()
            .reduce((all, system) => all + system.callHandler(SYSTEM_HANDLERS.getFuelSpace, null, 0), 0);
    }
    getFuelCost() {
        return this.ship.systems
            .getSystems()
            .reduce((all, system) => all + system.callHandler(SYSTEM_HANDLERS.getFuelRequirement, null, 0), 0);
    }
    payFuelCost() {
        ...
        if (fuelCost > shipFuel) {
            throw new Error("Unable to pay fuel cost. This should not happen since moves should be validated!:E");
        }
        const payFuel = () => {
            const tank = this.ship.systems
                .getSystems()
                .filter((system) => system.callHandler(SYSTEM_HANDLERS.getFuel, null, 0) > 0)
                .sort((a, b) => {
