...
class InterceptorStrategy extends ShipSystemStrategy {
    ...
    getInterceptChance({ target, torpedoFlight, }) {
        ...
        if (torpedoFlight instanceof TorpedoFlightForIntercept) {
            ...
            distance = new TorpedoFlightForIntercept(torpedoFlight, target).getClosestDistanceTo(ship);
        }
        const rangeModifier = this.getSystem().callHandler(SYSTEM_HANDLERS.getRangeModifier, {
            distance: distance,
        }, 0) *
            (1 + torpedoFlight.torpedo.getEvasion() / 10);
        const fireControl = this.getSystem().callHandler(SYSTEM_HANDLERS.getFireControl, null, 0);
        ...
        return new WeaponHitChance({
            ...
            rollingPenalty,
            result: Math.round(torpedoHitSize + rangeModifier + fireControl + ccew * 5 + rollingPenalty),
            absoluteResult: torpedoHitSize +
                rangeModifier +
                fireControl +
                ccew * 5 +
                rollingPenalty,
