...
class StandardRangeStrategy extends ShipSystemStrategy {
    ...
    getRangeModifier({ distance: currentRange }) {
        const exactModifier = this.rangesAndPenalties.find(({ range }) => range === currentRange);
        if (exactModifier) {
        ...
        const difference = end.modifier -
            start.modifier;
        const distance = end.range - start.range;
        return Math.round(start.modifier +
            (difference * (currentRange - start.range)) /
                distance);
    }
}
