...
class SystemDamage {
    ...
    hasCritical(object) {
        switch (typeof object) {
            case "object":
                return this.criticals.some((crit) => crit.constructor === object.constructor);
            case "function":
    ...
    getTotalDamage() {
        const damage = this.entries.reduce((acc, entry) => acc + entry.getDamage(), 0);
    ...
    advanceTurn(turn) {
        ...
        this.criticals = this.criticals.filter((critical) => !critical.isFixed(this.system));
    }
}
