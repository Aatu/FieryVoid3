...
import { MovementOrder, ThrustBill, } from "./index";
class MovementResolver {
    ...
    roll(commit = true) {
        ...
        if (rollMove) {
            ...
            rollMove = new MovementOrder(null, "roll" /* MOVEMENT_TYPE.ROLL */, endMove.position, endMove.velocity, endMove.facing, endMove.rolled, this.turn, endMove.rolled ? false : true);
    ...
    evade(step, commit = true) {
        ...
        if (evadeMove) {
            ...
            evadeMove = new MovementOrder(null, "evade" /* MOVEMENT_TYPE.EVADE */, endMove.position, endMove.velocity, endMove.facing, endMove.rolled, this.turn, 1);
        }
        const playerAdded = this.ship.movement
            .getMovement()
            .filter((move) => move.isPlayerAdded() && !move.isRoll() && !move.isEvade());
        const nonPlayerAdded = this.ship.movement
            .getMovement()
            .filter((move) => !move.isPlayerAdded() || move.isRoll());
        const movements = evadeMove.value === 0
    ...
    pivot(pivotDirection, commit = true) {
        ...
        const pivotMove = new MovementOrder(null, "pivot" /* MOVEMENT_TYPE.PIVOT */, lastMove.position, lastMove.velocity, addToHexFacing(lastMove.facing, pivotDirection), lastMove.rolled, this.turn, pivotDirection);
        const movements = this.ship.movement.getMovement();
        if (lastMove.isPivot() && lastMove.value !== pivotDirection) {
            movements.pop();
        }
        else {
            if (this.ship.maxPivots !== null &&
    ...
    thrust(direction, commit = true) {
        const lastMove = this.ship.movement.getLastMove();
        const thrustMove = new MovementOrder(null, "speed" /* MOVEMENT_TYPE.SPEED */, lastMove.position, lastMove.getHexVelocity().moveToDirection(direction), lastMove.facing, lastMove.rolled, this.turn, direction);
        let movements = this.ship.movement.getMovement();
