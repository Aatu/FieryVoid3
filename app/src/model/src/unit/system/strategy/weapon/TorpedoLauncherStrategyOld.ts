...
export type SerializedTorpedoLauncher = {
  ...
  changeAmmo: { ammo: TorpedoType; from: number } | null;
...
class TorpedoLauncherStrategy extends ShipSystemStrategy {
  ...
  public changeAmmo: { ammo: TorpedoType; from: number } | null;
  ...
  deserialize(
    ...
    this.loadedTorpedo = launcherData.loadedTorpedo
      ? createTorpedoInstance(launcherData.loadedTorpedo as TorpedoType)
      : null;
  ...
  launchTorpedo(
    ...
    let newAmmo: { ammo: TorpedoType; from: number } | null = null;
    ...
    if (same) {
      ...
      newAmmo = {
        ammo: same.object.constructor.name as TorpedoType,
        from: cargoBay.id,
      };
      ...
      newAmmo = {
        ammo: newTorpedo.constructor.name as TorpedoType,
        from: cargoBay.id,
      };
    }
  ...
  loadAmmo({ ammo, launcherIndex }: { ammo: Torpedo; launcherIndex: number }) {
    ...
    if (
      ...
      this.turnsLoaded = 0;
      this.changeAmmo = {
        ammo: ammo.constructor.name as TorpedoType,
        from: system.id,
      };
      this.loadedTorpedo = ammo;
    }
  ...
  receiveAmmoChange(changeAmmo: { ammo: TorpedoType; from: number }) {
