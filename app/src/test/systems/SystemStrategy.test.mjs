...
import ShipSystemStrategy from "../../model/unit/system/ShipSystemStrategy.mjs";

test("ShipSystem calls its strategies", test => {
  const testStrategy1 = new ShipSystemStrategy().addHandler(
    "onTest",
    (system, payload, response) => {
  ...
  const testStrategy2 = new ShipSystemStrategy().addHandler(
    "onTest",
    (system, payload, response) => {
      const number = response.number || 0;
