...
import { OutputReduced6 } from "../../model/unit/system/criticals/index";
...
const compareMovements = (test, moves1, moves2) => {
  test.deepEqual(
    moves1.map((move) => move.clone().setRequiredThrust(null).setIndex(0)),
    moves2.map((move) => move.clone().setRequiredThrust(null).setIndex(0))
  );
};
...
test("Boosted movements get deleted when thruster deboosts", (test) => {
  ...
  thruster.callHandler("boost");
  thruster.callHandler("boost");
  thruster.callHandler("boost");

  test.true(movementService.canThrust(ship, 0));
  test.is(ship.movement.getMovement().length, 5);

  movementService.thrust(ship, 0);

  thruster.callHandler("deBoost");
  test.is(ship.movement.getMovement().length, 5);
  test.falsy(movementService.canThrust(ship, 0));
});
