...
import { combatLogClasses } from "@fieryvoid3/model/src/combatLog/combatLogClasses";
const {
...
type CombatLogShipMovement = InstanceType<typeof CombatLogShipMovement>;
type CombatLogGroupedWeaponFire = InstanceType<typeof CombatLogGroupedWeaponFire>;
type CombatLogTorpedoLaunch = InstanceType<typeof CombatLogTorpedoLaunch>;
type CombatLogGroupedTorpedoAttack = InstanceType<typeof CombatLogGroupedTorpedoAttack>;
...
class ReplayTurnActions extends AnimationUiStrategy {
  ...
  async newTurn({ gameDatas }: { gameDatas: GameData[] }) {
    ...
    const entires = gameData.combatLog.getForReplay();

    entires.forEach((combatLogEntry) => {
